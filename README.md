...
## Legacy bundles (`ThePornDB*.bundle/`)

The `.bundle` directories in this repo are the original Plex plug-in agents.
They no longer load: Plex Media Server 1.43 removed the plug-in framework they
depend on. They are kept for reference — the matching logic, preference names
and TPDB field usage in `tpdb_provider/` are ported from them.

Nothing in those directories is used at runtime.
